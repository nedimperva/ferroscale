...
import type { QuickParseIssue, QuickWeightRequest, QuickWeightResponse } from "./types";
...
function mapIssues(issues: { field: string; message: string }[]): QuickParseIssue[] {
...
export function calculateQuickWeight(request: QuickWeightRequest, outputDecimals = DEFAULT_OUTPUT_DECIMALS): QuickWeightResponse {
  ...
  return {
    ...
    result: {
      ...
      selectedSizeId: request.selectedSizeId,
      manualDimensionsMm: request.manualDimensionsMm,
      quantity: request.quantity,
...
export function calculateQuickFromQuery(rawQuery: string, outputDecimals = DEFAULT_OUTPUT_DECIMALS): QuickWeightResponse {
