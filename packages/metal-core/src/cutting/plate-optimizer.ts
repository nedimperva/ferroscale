 ...
 * Supports standard European master sheet formats (1000×2000, 1250×2500, 1500×3000, 2000×6000, or custom)
 * with full 2D area awareness, 90° rotation, laser/saw blade kerf, and edge trim margins.
 */
export function optimizePlateCutList(
  ...
  function createNewSheet(sheetOption: PlateSheetOption): WorkingSheet {
    // Standard convention: X = length (longer), Y = width (shorter)
  ...
  for (const item of unplaced) {
    ...
    // Piece dimensions: normal vs rotated
    ...
    if (!bestPlacement) {
      ...
      if (allowRotation) {
        ...
        if (!canFit1 && canFit2) {
          ...
          // Pick the orientation that packs tighter against the short side
  ...
  // Build patterns and calculate area metrics
