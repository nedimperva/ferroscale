...
export const SaveToProjectModal = memo(function SaveToProjectModal({
  ...
  /* Only use the custom drawer behavior on desktop (mobile uses Vaul) */
  useDrawerBehavior(!isMobile && open, closeAndReset);
  ...
  const body = (
    <>
      {/* Feedback banner */}
      {feedback && (
        <div className="mb-3 rounded-lg border border-green-border bg-green-surface px-4 py-2.5 text-center text-sm font-medium text-green-text">
  ...
  /* ── Desktop: right-side drawer (original) ── */
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 z-40 bg-overlay transition-opacity duration-300 ${open ? "opacity-100" : "pointer-events-none opacity-0"
          }`}
      ...
      <aside
        aria-label={t("title")}
        className={`fixed inset-y-0 right-0 z-50 flex w-[400px] max-w-[95vw] flex-col bg-surface-raised shadow-xl transition-transform duration-300 ease-in-out ${open ? "translate-x-0" : "translate-x-full"
          }`}
      >
        {/* Header */}
