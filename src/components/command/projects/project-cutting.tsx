    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 text-xs flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-muted">
            {pattern.formatLabel} · {t("cutting.cutsCount", { count: pattern.cuts.length })}
          </span>
        </div>
        <div className="flex items-center gap-3 font-mono text-[11.5px]">
          <span className="text-muted">
...
export function ProjectCutting({ project, compact }: ProjectCuttingProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Profile Groups Selector */}
      <div className="space-y-1.5">
        <label className="fs-track-label text-[10px] font-bold text-muted uppercase">
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: isSelected ? "1px solid var(--accent)" : "1px solid var(--border-faint)",
                }}
              >
                <span>{grp.kind === "2d_plate" ? "📐 " : "📏 "}{grp.label}</span>
                <span
                  className="font-mono text-[10.5px] px-1.5 py-0.2 rounded-md"
                  style={{
                    background: isSelected ? "rgba(0,0,0,0.15)" : "var(--surface-inset)",
                    color: isSelected ? "var(--accent-contrast)" : "var(--muted)",
                  }}
                >
        ...
        {!is2D ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="space-y-1 sm:col-span-2">
              <span className="text-[10px] font-bold tracking-[1.1px] text-muted uppercase">
              ...
              <div className="flex gap-1.5 flex-wrap pt-0.5">
                {STOCK_PRESETS.map((p) => (
                  <button
                    ...
                    className="h-8 px-2.5 rounded-lg text-xs font-semibold cursor-pointer transition-colors"
                    style={{
                      background: stockChoice === p.value ? "var(--surface)" : "var(--surface-inset)",
                      border: stockChoice === p.value ? "1px solid var(--accent)" : "1px solid var(--border-faint)",
                ...
                <button
                  ...
                  className="h-8 px-2.5 rounded-lg text-xs font-semibold cursor-pointer transition-colors"
                  style={{
                    background: stockChoice === 0 ? "var(--surface)" : "var(--surface-inset)",
                    border: stockChoice === 0 ? "1px solid var(--accent)" : "1px solid var(--border-faint)",
                ...
                {stockChoice === 0 && (
                  <input
                    ...
                    className="h-8 w-24 px-2 rounded-lg text-xs font-mono bg-[var(--surface)] border border-[var(--border-faint)] text-foreground"
                    placeholder="mm"
                  />
                )}
              </div>
            ...
            <div className="flex gap-3 items-center">
              <div className="space-y-1 flex-1">
                ...
                <input
                  ...
                  className="h-8 w-full px-2 rounded-lg text-xs font-mono bg-[var(--surface)] border border-[var(--border-faint)] text-foreground"
                />
              </div>

              <div className="space-y-1 flex-1">
                <span className="text-[10px] font-bold tracking-[1.1px] text-muted uppercase">
                  {t("cutting.minOffcut")} (mm)
                </span>
                <input
                  type="number"
                  min={0}
          ...
          <div className="space-y-3">
              ...
              <div className="flex gap-1.5 flex-wrap pt-0.5">
                {STANDARD_EURO_SHEET_FORMATS.map((fmt, idx) => (
                  <button
                    ...
                    className="h-8 px-2.5 rounded-lg text-xs font-semibold cursor-pointer transition-colors"
                    style={{
                      background: selectedPlateFormatIndex === idx ? "var(--surface)" : "var(--surface-inset)",
                      border: selectedPlateFormatIndex === idx ? "1px solid var(--accent)" : "1px solid var(--border-faint)",
                      color: selectedPlateFormatIndex === idx ? "var(--accent-text)" : "var(--foreground)",
                    }}
                  >
                    {fmt.widthMm} × {fmt.lengthMm} mm
                  </button>
                ))}
                <button
                  type="button"
                  ...
                  className="h-8 px-2.5 rounded-lg text-xs font-semibold cursor-pointer transition-colors"
                  style={{
                    background: selectedPlateFormatIndex === -1 ? "var(--surface)" : "var(--surface-inset)",
                    border: selectedPlateFormatIndex === -1 ? "1px solid var(--accent)" : "1px solid var(--border-faint)",
            ...
            {selectedPlateFormatIndex === -1 && (
              <div className="flex gap-2 items-center">
                <input
                  ...
                  className="h-8 w-28 px-2 rounded-lg text-xs font-mono bg-[var(--surface)] border border-[var(--border-faint)] text-foreground"
                />
            ...
            <div className="flex gap-4 items-center flex-wrap pt-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[11px] text-muted">{t("cutting.laserKerf")} (mm):</span>
                <input
                  type="number"
                  min={0}
                  max={10}
                  ...
                  value={kerfMm}
                  onChange={(e) => setKerfMm(Number(e.target.value))}
                  className="h-7 w-16 px-1.5 rounded text-xs font-mono bg-[var(--surface)] border border-[var(--border-faint)] text-foreground"
                />
              </div>

              <div className="flex items-center gap-1.5">
                <span className="text-[11px] text-muted">{t("cutting.edgeTrim")} (mm):</span>
                <input
                  type="number"
                  min={0}
                  max={50}
              ...
              <label className="flex items-center gap-2 cursor-pointer text-xs text-foreground">
                ...
                />
                <span>{t("cutting.allowRotation")}</span>
              </label>
            </div>
          </div>
        )}
      ...
      {!is2D && barOptResult && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          <StatTile
            ...
            value={`${barOptResult.totalStockBars}`}
            sub={`${barOptResult.totalPiecesCount} ${t("cutting.cutsCount", { count: barOptResult.totalPiecesCount })}`}
            tone="accent"
          />
          <StatTile
            label={t("cutting.used")}
            value={`${(barOptResult.totalCutLengthMm / 1000).toFixed(2)} m`}
            sub={`of ${(barOptResult.totalStockLengthMm / 1000).toFixed(1)} m total`}
          />
          <StatTile
            label={t("cutting.scrapLoss")}
            value={`${(barOptResult.totalScrapMm / 1000).toFixed(2)} m`}
            sub={`${barOptResult.scrapPercent}% waste`}
          />
          <StatTile
      ...
      {is2D && plateOptResult && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          <StatTile
            ...
            value={`${plateOptResult.totalMasterSheets}`}
          ...
          <StatTile
            ...
            sub={plateOptResult.yieldPercent >= 75 ? "✨ Efficient nest" : "Standard nest"}
      ...
      <div className="space-y-3 pt-1">
        <div className="flex items-center justify-between gap-2">
          <h4 className="text-xs font-bold text-foreground">
            {t("cutting.cuttingPlan", { profile: activeGroup?.label ?? "" })}
          </h4>
          <button
            type="button"
            onClick={() => window.print()}
            className="h-8 px-3 rounded-xl text-xs font-semibold border border-[var(--border-faint)] bg-[var(--surface)] hover:bg-[var(--surface-raised)] text-foreground flex items-center gap-1.5 cursor-pointer"
          >
            <DeskIcon name="share" />
