...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: "2026-05-14",
    added: [
      "Numpad-native mobile calculator: live total weight headline, profile + material chip cards, length/pieces/price input strip, and a persistent 4x4 numpad that drives the active field. Tap the result card for the full breakdown.",
      "Dedicated profile and material picker bottom sheets on mobile, opened from the chip cards. Profile sheet: category row, profile-type grid, and size/dimension inputs. Material sheet: family chips and grade list with EN reference and density.",
      "Mobile result sheet rebuilt to match the numpad-native concept: big tabular weight headline with profile glyph, price plus EUR/kg subtitle, three quick-stat cards (per piece / per metre / length), a peek-to-full breakdown table, and a full-width terracotta Add to project action at the full snap.",
      "Settings opens with a Defaults applied hero card summarising the material, length unit, currency, waste and VAT used for every new calculation.",
      "First-run onboarding: three quick steps to pick a default material, starting profile, and length unit plus currency. Skipped or finished once per device; can be replayed from Settings.",
      "Mobile Settings rebuilt with grouped rounded section cards - Display, Defaults, and Power - each row a single tappable card with an accent-tinted icon and either a toggle or a sub-sheet picker.",
      "Projects mobile tab gains a hero card at the top of the screen showing the active project name, parts count, weight chip and cost chip.",
      "Desktop Workstation top bar (lg+): page title, live calculation context chip, Cmd+K quick-calc search trigger, and a primary New reset button.",
    ],
    changed: [
      "Cream/orange numpad-native palette applied globally - warmer background, softer surfaces, terracotta accent, refined borders and shadows. Dark mode tuned to a coherent warm-dark variant. Affects every screen.",
      "All shell-level shadows (sidebar, mobile header, bottom tab bar, result bar, result sheet, sticky sub-headers, hover tooltips) now use the warm-ink palette instead of the old slate base.",
      "Mobile bottom tab bar replaced by a hamburger button in the top header that opens a bottom-sheet menu listing Calculator, Projects, Settings, What's New, Report an issue, and Replay onboarding.",
      ...
      "Edge-swipe tab navigation removed - the mobile header is the single nav entry point now.",
      "Mobile calculator no longer renders the legacy inline profile/material/dimensions form below the new numpad - profile and material live in their own bottom sheets. Reverse calculator and inline pricing toggles still appear on desktop; mobile homes for them ship in a follow-up.",
      "Floating mobile result bar removed entirely. The calculator tab owns its own live result card; Projects and Settings are scroll-only on mobile.",
    ],
    fixed: [
    ...
    added_bs: [
      "Numpad-native mobilni kalkulator: zivi prikaz ukupne tezine, cipovi za profil i materijal, traka za duzinu/komade/cijenu i stalno vidljiv 4x4 numpad koji upravlja aktivnim poljem. Dodirni rezultat za detaljan pregled.",
      "Posebni bottom sheet-ovi za izbor profila i materijala na mobilnom, otvaraju se sa cipova. Sheet profila: kategorije, mreza profila i unos velicine/dimenzija. Sheet materijala: cipovi porodica i lista kvaliteta sa EN referencom i gustocom.",
      "Mobilni sheet rezultata redizajniran prema numpad-native konceptu: krupan tabularni broj tezine sa glifom profila, cijena uz EUR/kg, tri kartice brzih statistika (po komadu / po metru / duzina), tabela razrade peek/full i terracotta dugme Dodaj u projekat u punoj sirini.",
      "Postavke se otvaraju karticom Primijenjeni defaulti sa sazetkom materijala, jedinice duzine, valute, otpada i PDV-a koji se koriste za svaku novu kalkulaciju.",
      "Onboarding pri prvom pokretanju: tri kratka koraka za odabir materijala, profila i jedinice duzine plus valute. Pojavljuje se jednom po uredjaju; moze se ponoviti iz Postavki.",
      "Mobilne Postavke su rekonstruisane kao grupisane zaobljene sekcije - Prikaz, Defaulti i Napredno - svaki red je jedna dodirljiva kartica sa ikonom u boji akcenta i toggle-om ili sub-sheet izborom.",
      "Projekti na mobilnom dobijaju hero karticu na vrhu sa imenom aktivnog projekta, brojem dijelova, cipom tezine i cipom cijene.",
      "Desktop Workstation top bar (lg+): naslov stranice, zivi cip konteksta kalkulacije, Cmd+K prekidac za brzu pretragu i primarno dugme Novo za reset.",
    ],
    changed_bs: [
      "Cream/orange numpad-native paleta primijenjena globalno - topla pozadina, mekse povrsine, terracotta akcent, profinjene ivice i sjene. Tamni mod podesen na uskladjeni topli mracni varijant. Utice na svaki ekran.",
      "Sve sjene shell-a (bocna traka, mobilni header, donja tab traka, result bar, result sheet, sticky sub-headeri, tooltip-i) sada koriste paletu toplog mastila umjesto starije slate baze.",
      "Donja tab traka na mobilnom zamijenjena hamburger dugmetom u gornjem headeru koje otvara bottom-sheet meni sa Kalkulator, Projekti, Postavke, Novosti, Prijavi problem i Pokreni uvod ponovo.",
      "Sabloni / Sacuvano su skriveni tokom redizajna. Ruta sacuvanog preusmjerava na kalkulator, a postojeci sabloni ostaju sacuvani u localStorage-u za buduci povratak.",
      "Uporedi vise nije primarna navigacijska stavka. Compare tray vezan za rezultat i akcije u result sheet-u sada otvaraju puni Compare drawer.",
      "Navigacija povlacenjem sa ivica je uklonjena - mobilni header je sada jedini ulaz u navigaciju.",
      "Mobilni kalkulator vise ne prikazuje stari ugradjeni profil/materijal/dimenzije ispod novog numpada - profil i materijal su sada u svojim bottom sheet-ovima. Obrnuti kalkulator i inline pricing toggle-i jos uvijek su na desktopu; mobilne lokacije slijede.",
      "Floating mobilni result bar potpuno uklonjen. Kalkulator tab ima svoju karticu sa zivim rezultatom; Projekti i Postavke su na mobilnom samo za skroll.",
    ],
    fixed_bs: [
      "Mobilni numpad sada drzi promjenu jedinice duzine samo u Postavkama/onboardingu, a tipke jedinica u kalkulatoru zamjenjuje akcijama Ocisti, Dalje i Gotovo uz sinhronizovan Length cip nakon vanjskih promjena jedinice.",
    ],
  },
  {
    version: "2.6.0",
    date: "2026-05-14",
    changed: [
      "Redesigned the default workspace around a bare calculator-first layout with a compact top bar, overflow menu, flatter controls, and a mobile result summary without the bottom tab bar.",
      ...
      "Grouped the overflow menu into Workspace, Tools, and Help sections for easier scanning, and regrouped it around a Library section (Saved, Projects, Compare) so the three stashes feel like one concept.",
      ...
      "Collapsed the calculator profile picker into a single button that expands the category and sub-type pills on demand - saves vertical space on every layout. A pin option keeps the grid open across selections and outside clicks; the preference is persisted.",
      "Added Save as template and Add to project actions on Compare items, so a comparison can land in the persistent library without re-entering it in the calculator. (Save as template later removed from Compare; Add to project stays.)",
      ...
      "Removed the toast notification system entirely. State changes still surface through inline UI and sync status badges.",
      "Rebuilt the result modal in a minimal, typography-led style: single warm-tan accent for the primary value, neutral label-on-left/value-on-right metric rows, an inline context line, and a flatter ghost-style action grid. The modal shell matches the floating result chip - rounded-xl, lighter shadow, corner close button instead of a titled header bar.",
    ],
    changed_bs: [
      "Redizajniran podrazumijevani radni prostor oko jednostavnog calculator-first layouta sa kompaktnom gornjom trakom, overflow menijem, ravnijim kontrolama i mobilnim sazetkom rezultata bez donje tab trake.",
      "Uklonjen duplirani Calculator naslov sekcije sa desktopa, multi-column i mobilnih layouta - gornja traka vec prenosi identitet stranice.",
      "Overflow meni grupisan u sekcije Radni prostor, Alati i Pomoc radi laksh pregleda, te pregrupisan oko sekcije Biblioteka (Sabloni, Projekti, Uporedi) da tri spremista djeluju kao jedan koncept.",
      "Desktop sada koristi jednu centriranu kolonu (forma gore, rezultat inline ispod) umjesto fiksne desne kolone od 360 px, tako da forma kalkulatora dobija znacajnu sirinu.",
      "Mobilni subtitle u gornjoj traci se skriva dok je rezultat prikazan, jer floating result bar vec nosi zive brojeve.",
      "Profil picker kalkulatora skupljen u jedno dugme koje otvara kategorije i pod-tipove na zahtjev - stedi vertikalni prostor na svakom layoutu. Opcija pin drzi mrezu otvorenu kroz odabire i klikove van; preferenca se cuva.",
      "Dodane akcije Sacuvaj kao sablon i Dodaj u projekat na Uporedi stavke, tako da poredjenje moze zavrsiti u trajnoj biblioteci bez ponovnog unosa u kalkulator. (Sacuvaj kao sablon kasnije uklonjeno sa Uporedi; Dodaj u projekat ostaje.)",
      "Siroki desktopi (>= 1280 px) prikazuju formu kalkulatora i rezultat jedno pored drugog, sa sticky i scroll-contained kolonom rezultata, tako da se stranica ne skrolla dok je rezultat vidljiv. Laptopi (1024-1279 px) zadrzavaju single-column layout.",
      "Zamijenjen inline desktop panel rezultata floating result cipom u donjem desnom uglu (isto na mobilnom i desktopu). Klikni cip za prosirivanje: bottom sheet na mobilnom, centrirani modal na desktopu.",
      "Uklonjena neiskoristena ResultActionsSheet komponenta; akcije rezultata su unutar prosirenog rezultata.",
      "Result overlay je sada centrirani modal na svakoj velicini ekrana (mobilni bottom sheet je penzionisan).",
      "Toast sistem obavjestenja u potpunosti uklonjen. Promjene stanja se i dalje prikazuju kroz inline UI i sync status oznake.",
      "Result modal rekonstruisan u minimalnom, tipografski vodjenom stilu: jedan topli tan akcent za primarnu vrijednost, neutralni redovi label-lijevo/vrijednost-desno, inline linija konteksta i ravnija ghost mreza akcija. Modal shell odgovara floating result cipu - rounded-xl, blaza sjena, dugme zatvaranja u uglu umjesto naslovne trake.",
    ],
  },
  {
    version: "2.5.2",
